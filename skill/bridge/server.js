...
// PWA shell assets (manifest, service worker, icons) — make the web client
// installable as a standalone app on Android Chrome and iOS Safari. All optional:
// a missing file just 404s and the page still works as a plain browser tab.
function loadAsset(rel) {
  try { return fs.readFileSync(new URL(rel, import.meta.url)); } catch { return null; }
}
const PWA_ASSETS = {
  "/manifest.webmanifest": { body: loadAsset("./manifest.webmanifest"), type: "application/manifest+json; charset=utf-8", cache: "no-cache" },
  "/sw.js":                { body: loadAsset("./sw.js"),                 type: "text/javascript; charset=utf-8",        cache: "no-cache" },
  "/icons/icon-192.png":          { body: loadAsset("./icons/icon-192.png"),          type: "image/png", cache: "public, max-age=604800" },
  "/icons/icon-512.png":          { body: loadAsset("./icons/icon-512.png"),          type: "image/png", cache: "public, max-age=604800" },
  "/icons/icon-maskable-512.png": { body: loadAsset("./icons/icon-maskable-512.png"), type: "image/png", cache: "public, max-age=604800" },
};
...
// Serve a PWA shell asset (public, no auth — same posture as the web client at /).
function handlePwaAsset(req, res) {
  const p = new URL(req.url, `http://${req.headers.host}`).pathname;
  const a = PWA_ASSETS[p];
  if (!a || !a.body) { res.writeHead(404); res.end("not found"); return; }
...
const routes = {
  "GET /": handleWebClient,
  "GET /manifest.webmanifest": handlePwaAsset,
  "GET /sw.js": handlePwaAsset,
  "GET /icons/icon-192.png": handlePwaAsset,
  "GET /icons/icon-512.png": handlePwaAsset,
